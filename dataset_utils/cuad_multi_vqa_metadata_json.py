...
doc_names = list(doc_image_names.keys())
...
for split_type, split_docs in dataset_docs.items():
    ...
    with open(f"{split_dir}/metadata.jsonl", "w") as metadata_file:
        for split_doc in split_docs:
            for image_name in doc_image_names[split_doc]:
                QAs = []
                if split_doc in cuad_vqa:
                    if image_name in cuad_vqa[split_doc]:
                        QAs = cuad_vqa[split_doc][image_name]
                metadata_file.write(
                    json.dumps(
                        {
